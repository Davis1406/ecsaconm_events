...
class Auth:
    ...
    def secure_access(self, permission_code, user_id):
        # Users with ADMIN_DASHBOARD bypass all other permission checks
        if permission_code != "ADMIN_DASHBOARD":
            admin_check = self.db.query(UserRole, User, RolePermission, Permission).filter(
                User.id == UserRole.user_id, User.id == user_id).filter(
                UserRole.role_id == RolePermission.role_id).filter(
                Permission.id == RolePermission.permission_id,
                Permission.permission_code == "ADMIN_DASHBOARD").first()
            if admin_check:
                return admin_check
