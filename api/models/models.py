...
class UserProfile(BaseWithSoftDelete):
    ...
    certificate_name = Column(String(300), nullable=True)
...
class Organiser(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    organiser = Column(String(200), nullable=False, unique=True)
    description = Column(Text, nullable=True)
    created_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=func.now(), onupdate=func.now())
    deleted_at = Column(TIMESTAMP(timezone=True), nullable=True)
    ...
    def __repr__(self):
        return f"<Organiser id={self.id}, organiser={self.organiser}>"
...
class Event(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("user.id"), nullable=False)
    org_unit_id = Column(Integer, ForeignKey("org_unit.id"), nullable=False)
    country_id = Column(Integer, ForeignKey("country.id"), nullable=False)
    event_type_id = Column(Integer, ForeignKey("event_type.id"), nullable=True)
    organiser_id = Column(Integer, ForeignKey("organiser.id"), nullable=True)
...
class Participant(Base):
    ...
    __table_args__ = (Index("ix_participant", "deleted_at"),)

    def __repr__(self):
        return f"<Participant id={self.id}, name={self.firstname} {self.lastname}>"
...
class EmailTemplate(Base):
    ...
    variables = Column(Text, nullable=True)  # comma-separated list of available variables
    created_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(
    ...
    def __repr__(self):
        return f"<EmailTemplate key={self.template_key}>"
