...
async def preview_gala_invitations(
    current_user: user_dependency,
    db: Session = Depends(get_db),
    auth_dependency: Auth = Depends(get_auth_dep),
    ...
    if body.event_id:
        q = q.filter(Registration.event_id == body.event_id)
    recipient_count = q.scalar() or 0
...
@router.post("/send_gala_invitations")
async def send_gala_invitations(
    current_user: user_dependency,
    db: Session = Depends(get_db),
    auth_dependency: Auth = Depends(get_auth_dep),
    background_tasks: BackgroundTasks = BackgroundTasks(),
    body: SendGalaInvitationsSchema = None,
):
    ...
    return {
        "sent": sent,
        "message": f"Gala dinner invitation queued for {sent} registrant(s), paid and unpaid alike.",
    }
