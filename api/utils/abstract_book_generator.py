...
Results/Conclusion sections bolded and indented for readability.

The official conference preface (cover, welcome messages, overview — supplied
as a ready-made PDF) is prepended verbatim, unmodified, ahead of the abstract
listing that this module generates.
"""
import os
import io
import re
...
PREFACE_PDF_PATH = os.path.join(ASSETS_DIR, "abstract_book_preface.pdf")
...
def _generate_listing_pdf(event_name: str, abstracts: list) -> bytes:
    """Builds just the abstract-listing pages (grouped by track), starting
    directly on the first page — no cover/welcome/overview content here,
    that comes from the verbatim preface PDF prepended in
    generate_abstract_book_pdf()."""
    buffer = io.BytesIO()
    W = A4[0] - 4 * cm
    ...
    def _banner(text_paragraph, height_pad=8, bg=_PINK):
    ...
    for track_idx, (track_name, items) in enumerate(sorted(tracks.items())):
        ...
        elements.append(_banner(Paragraph(_esc(track_name), track_s)))
...
def generate_abstract_book_pdf(event_name: str, abstracts: list, event_meta: dict = None) -> bytes:
    ...
    `event_meta` is accepted for compatibility but unused — the preface (with
    its own cover/dates/theme) is attached exactly as supplied, unmodified.

    Result = the verbatim preface PDF pages, followed immediately by the
    abstract listing (no extra pages in between).
    """
    listing_bytes = _generate_listing_pdf(event_name, abstracts)

    if not os.path.exists(PREFACE_PDF_PATH):
        return listing_bytes

    from pypdf import PdfReader, PdfWriter

    writer = PdfWriter()
    for page in PdfReader(PREFACE_PDF_PATH).pages:
        writer.add_page(page)
    for page in PdfReader(io.BytesIO(listing_bytes)).pages:
        writer.add_page(page)
