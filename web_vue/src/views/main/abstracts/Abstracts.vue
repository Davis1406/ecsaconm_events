<template>
  ...
  <!-- ══════════════════════════════════════════════════════════════════════ -->
  <!-- Attendance Confirmation Email Modal                                   -->
  <!-- ══════════════════════════════════════════════════════════════════════ -->
  <Teleport to="body">
    <div v-if="confirmation.open"
      class="fixed inset-0 z-50 flex items-center justify-center p-4"
      @click.self="closeConfirmationModal">
      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="closeConfirmationModal"></div>
      ...
      <div class="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col" style="height: 88vh;">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="font-semibold text-gray-800 text-base">Attendance Confirmation Email</h2>
              <p class="text-xs text-gray-400">Sent to unregistered presenters with a personal confirmation link.</p>
            </div>
          </div>
          <button @click="closeConfirmationModal" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div class="flex items-center gap-3 p-4 rounded-xl border border-gray-100 bg-gray-50">
            <div class="flex-1">
              <p class="text-sm font-semibold text-gray-800">
                <span class="text-xl font-bold" style="color: rgb(254,80,103);">{{ confirmation.recipientCount }}</span>
                presenter{{ confirmation.recipientCount !== 1 ? 's' : '' }} will receive this email
              </p>
              <p class="text-xs text-gray-400 mt-0.5">Presenters with accepted abstracts who haven't registered yet.</p>
            </div>
            <button @click="loadConfirmationRecipients" class="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 transition">
              Refresh
            </button>
          </div>

          <!-- Feedback -->
          <div v-if="confirmation.result" class="p-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm">
            {{ confirmation.result }}
          </div>
          <div v-if="confirmation.error" class="p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm">
            {{ confirmation.error }}
          </div>
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1 uppercase tracking-wide">Email Subject</label>
            <input v-model="confirmation.subject" type="text"
              class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-pink-400" />
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-xs font-semibold text-gray-600 uppercase tracking-wide">HTML Body</label>
              <button @click="confirmation.editMode = confirmation.editMode === 'edit' ? 'preview' : 'edit'"
                class="text-xs px-3 py-1 rounded-lg border transition"
                :class="confirmation.editMode === 'preview'
                  ? 'border-pink-400 text-pink-600 bg-pink-50'
                  : 'border-gray-200 text-gray-500 hover:border-gray-300'">
                {{ confirmation.editMode === 'preview' ? '✎ Edit HTML' : '👁 Preview' }}
              </button>
            </div>

            <div v-if="confirmation.editMode === 'preview'" class="border border-gray-200 rounded-xl overflow-hidden bg-white">
              <div class="flex items-center justify-between px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                <span class="text-xs font-semibold text-gray-500">{{ confirmation.subject || 'Email subject' }}</span>
                <span class="text-[10px] text-gray-400 uppercase tracking-wide">Preview · {{ confirmation.recipientCount }} recipient{{ confirmation.recipientCount !== 1 ? 's' : '' }}</span>
              </div>
              <iframe :srcdoc="renderConfirmationPreview(confirmation.body_html)" style="width:100%; height:440px; border:none;"></iframe>
            </div>

            <textarea v-else v-model="confirmation.body_html" rows="24"
              class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm font-mono focus:outline-none focus:border-pink-400"
              style="resize: vertical;"></textarea>
            ...
            <p class="text-xs text-gray-400 mt-1">
              ...
              <code class="bg-gray-100 px-1 rounded">event_name</code>
              <code class="bg-gray-100 px-1 rounded">abstract_title</code>
              <code class="bg-gray-100 px-1 rounded">form_link</code>
              <code class="bg-gray-100 px-1 rounded">year</code>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-100 flex-shrink-0">
          <div class="text-xs text-gray-400">
            {{ confirmation.loading ? 'Loading…' : (confirmation.recipientCount + ' recipient(s)') }}
          </div>
          <div class="flex gap-3">
            <button @click="resetConfirmationTemplate"
              class="px-5 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-50 transition">
              Reset
            </button>
            <button @click="saveConfirmationTemplate" :disabled="confirmation.saving || confirmation.loading"
              ...
              {{ confirmation.saving ? 'Saving…' : 'Save Template' }}
            </button>
            <button @click="sendConfirmationEmails" :disabled="confirmation.sending || confirmation.loading || !confirmation.recipientCount"
              class="px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
              style="background-color: rgb(254,80,103);">
              {{ confirmation.sending ? 'Sending…' : `Send to ${confirmation.recipientCount}` }}
            </button>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // Attendance confirmation form modal (preview / edit / send)
      confirmation: {
        open: false, loading: false, saving: false, sending: false,
        recipientCount: 0,
        subject: '', body_html: '',
  ...
  methods: {
    ...
    async openConfirmationModal() {
      ...
      await Promise.all([this.loadConfirmationTemplate(), this.loadConfirmationRecipients()])
      this.confirmation.loading = false
    },
    closeConfirmationModal() {
      this.confirmation.open = false
    },
    async loadConfirmationRecipients() {
      try {
        const res = await axios.get(`${this.apiUrl}/attendance-form/recipients`, {
          headers: { Authorization: `Bearer ${this.accessToken}` },
        })
        this.confirmation.recipientCount = Array.isArray(res.data) ? res.data.length : 0
      } catch (e) {
        this.confirmation.error = e.response?.data?.detail || 'Failed to load recipients.'
      }
    },
    async loadConfirmationTemplate() {
      try {
        const res = await axios.get(`${this.apiUrl}/email_templates/attendance_confirmation_form`, {
          headers: { Authorization: `Bearer ${this.accessToken}` },
        })
        this.confirmation.subject = res.data.subject || ''
        this.confirmation.body_html = res.data.body_html || ''
        this.confirmation.originalSubject = this.confirmation.subject
        this.confirmation.originalBody = this.confirmation.body_html
        this.confirmation.editMode = 'preview'
      } catch (e) {
        this.confirmation.error = e.response?.data?.detail || 'Failed to load template.'
      }
    },
    renderConfirmationPreview(bodyHtml) {
      const sample = {
        subject: this.confirmation.subject || 'Confirm Your Attendance',
        firstname: 'Jane Presenter',
        lastname: 'Example',
        event_name: 'ECSACONM Scientific Conference',
        abstract_title: 'Sample Accepted Abstract Title',
        form_link: `${window.location.origin}/#/attendance-form/sample-token`,
        year: new Date().getFullYear(),
        username: this.confirmation.subject || 'Confirm Your Attendance',
      }
    ...
    resetConfirmationTemplate() {
      this.confirmation.subject = this.confirmation.originalSubject
      this.confirmation.body_html = this.confirmation.originalBody
      this.confirmation.editMode = 'preview'
      this.confirmation.result = ''
      this.confirmation.error = ''
    },
    async saveConfirmationTemplate() {
      ...
      try {
        await axios.put(
          `${this.apiUrl}/email_templates/attendance_confirmation_form`,
          { subject: this.confirmation.subject, body_html: this.confirmation.body_html },
          { headers: { Authorization: `Bearer ${this.accessToken}` } }
        )
        this.confirmation.originalSubject = this.confirmation.subject
        this.confirmation.originalBody = this.confirmation.body_html
        this.confirmation.result = 'Template saved. Changes will be used on send.'
      } catch (e) {
        this.confirmation.error = e.response?.data?.detail || 'Failed to save template.'
      } finally {
    ...
    async sendConfirmationEmails() {
      ...
      try {
        ...
        this.confirmation.result = res.data?.message || `Confirmation forms emailed to ${res.data?.sent || 0} presenter(s).`
        await this.loadConfirmationRecipients()
      } catch (e) {
