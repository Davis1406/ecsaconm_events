  ...
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col w-5/12 w-full space-y-4" @submit.prevent="editUser" method="POST">
        ...
        <p v-if="errorMsg" class="text-sm px-3 py-2 rounded-lg bg-red-50 text-red-600">
          {{ errorMsg }}
        </p>
        <div class="flex flex-row space-x-4">
          <button type="submit" :disabled="isLoading" class="mt-2 px-4 py-2 text-white-100 bg-bondi-blue hover:bg-bondi-blue-400 rounded-2xl disabled:opacity-50">
  ...
  methods: {
    async editUser() {
      ...
      try {
        ...
        this.errorMsg = error.response?.data?.detail || "Failed to update user. Please try again.";
        this.isLoading = false;
      }
    },
    async getUser() {
